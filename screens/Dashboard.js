import { Dimensions, StyleSheet, Text, View, TouchableOpacity, Image, ScrollView, BackHandler, Alert } from 'react-native'
...
const Dashboard = (props) => {
  ...
  useEffect(() => {
    EventRegister.emit('FarmerInfoData');
    setInterval(() => {
      getData();
      getCampName()
    }, 100);
  ...
  const getData = async () => {
    ...
    await axios
      ...
      .then(function (response) {
        ...
        let result = response.data.data.farmers[response.data.data.farmers.length - 1].farmerName;
    ...
    <View style={{ backgroundColor: '#ffffff', flex: 1 }} >
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={{ paddingTop: 20 }}>
        ...
        <View style={styles.dashboardCard}>
          <DashboardCard title={"Total Leads"} value={totalLeads !== null ? totalLeads : 'NA'} onPress={() => { props.navigation.navigate('AllLead') }} />
          <DashboardCard title={"Camp attended"} value={'50+'} />
        </View>
        <View style={styles.dashboardCard}>
          <DashboardCard title={"Active Vet Camps"} value={campCount !== null ? campCount : 'NA'} />
        ...
        <View style={{ marginHorizontal: 10, marginTop: 30 }}>
          ...
          <TouchableOpacity style={styles.recentlyAddedTextContainer} onPress={() => { props.navigation.navigate('AllLead') }}>
            ...
            <View style={styles.recentlyAddedIcon}><Icon name="angle-right" size={14} color="#058cb2" /></View>
          </TouchableOpacity>
        </View>
